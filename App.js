class App extends React.Component {
	...
	randomize() {
		let newColors = this.state.colors.map((color, idnex) => {
			if (!color.isLocked) {
				return {
					...color,
		...
		return (
			<div>
				<div className="text-center bg-dark fixed-top">
					<button className="btn btn-secondary m-1" onClick={() => this.randomize()}>
						RANDOMIZE COLORS
					</button>
				</div>
				<div className="w-100 d-flex" style={{height: "100vh"}}>
					{this.state.colors.map((color, index) => (
						<Color color={color} key={index} toggle={() => this.toggleLock(index)} />
					))}
				</div>
			</div>
		);
